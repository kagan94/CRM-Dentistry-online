...
class ServicesController extends Controller
{
	...
	public function actionCreate()
	{
		...
		{
			$_POST['Services']['date_added'] = date("Y-m-d");
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Services']))
		{	
			$_POST['Services']['date_modifidied'] = date("Y-m-d");

			if($model->parent_id == 0 && (int)$_POST['Services']['parent_id'] != 0){

				$criteria=new CDbCriteria();
				$criteria->condition = 'parent_id = '.$id;

				Services::model()->updateAll(array('parent_id'=>0), $criteria);
			}

			if((int)$_POST['Services']['parent_id'] == 0){
				$_POST['Services']['code'] = 0;
				$_POST['Services']['price'] = NULL;
			} else {
